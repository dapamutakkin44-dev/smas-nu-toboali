    ...
    <main className="min-h-screen bg-[#f8fafc] overflow-x-hidden font-sans text-slate-800">
      ...
      <nav className="fixed top-0 w-full z-[100] bg-white/90 backdrop-blur-xl border-b border-emerald-100 py-4 px-6 md:px-12 flex justify-between items-center shadow-md">
        <div className="flex items-center gap-4">
          <button onClick={() => setIsMenuOpen(true)} className="p-2 text-emerald-900 hover:bg-emerald-50 rounded-xl transition-all flex items-center gap-2 group">
            <Menu size={28} />
        ...
        <Link href="/ppdb" className="hidden md:block bg-emerald-600 text-white px-8 py-2.5 rounded-full no-underline hover:bg-emerald-700 shadow-lg shadow-emerald-100 transition-all font-black text-[10px] uppercase tracking-widest border-none">PPDB 2026</Link>
      </nav>

      {/* 2. RUNNING TEXT (BIAR GACOR) */}
      <div className="fixed top-[72px] md:top-[78px] w-full z-40 bg-amber-400 py-2 overflow-hidden whitespace-nowrap border-b border-amber-500/20">
      ...
      {/* 4. HERO SECTION */}
      <section className="relative min-h-screen flex items-center justify-center pt-32 overflow-hidden bg-[radial-gradient(circle_at_top_right,_var(--tw-gradient-stops))] from-emerald-50 via-white to-white text-left">
      ...
      {/* 5. SAMBUTAN KEPSEK (KEMBALI & GAHAR) */}
      <section className="py-24 px-6 bg-white">
        <div className="max-w-6xl mx-auto flex flex-col md:flex-row items-center gap-16 text-left">
          ...
          <div className="w-full md:w-1/2">
            <p className="text-emerald-600 font-black uppercase tracking-[0.4em] text-[10px] mb-4">Welcome Message</p>
            ...
            <div className="pt-6 border-t border-slate-100"><p className="font-black text-xl italic text-slate-800 mb-0">Azharudin, S.Pd., M.E., Gr.</p><p className="text-emerald-600 font-bold text-xs uppercase tracking-widest">Kepala Sekolah</p></div>
          </div>
        </div>
      </section>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-8">
            {[
              ...
              <div key={idx} className="group bg-white/5 border border-white/10 p-10 rounded-[40px] hover:bg-emerald-600 transition-all duration-500">
                ...
                <h3 className="font-black text-lg italic leading-tight mb-2 tracking-tighter h-12 flex items-center justify-center underline-none">{staff.nama}</h3>
