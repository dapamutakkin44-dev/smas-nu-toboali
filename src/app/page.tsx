...
export default function HomePage() {
  ...
  return (
    <main className="min-h-screen bg-[#f8fafc] overflow-x-hidden font-sans text-slate-800">
          ...
          <>
            ...
            >
              <div className="flex justify-between items-center mb-10">
                ...
                <button onClick={() => setIsMenuOpen(false)} className="text-emerald-200 hover:text-white transition-colors bg-transparent border-none cursor-pointer">
                  <X size={32} />
                </button>
              </div>
              ...
              <div className="space-y-2">
                ...
                <div className="pt-6 pb-4 space-y-3 border-t border-emerald-800/50">
                   ...
                   <Link onClick={() => setIsMenuOpen(false)} href="/profil/struktur" className="flex items-center gap-3 text-lg font-bold text-emerald-100 no-underline hover:text-white transition-all"><Users size={18} className="text-amber-400" /> Struktur Organisasi</Link>
                   <Link onClick={() => setIsMenuOpen(false)} href="/profil/visi-misi" className="flex items-center gap-3 text-lg font-bold text-emerald-100 no-underline hover:text-white transition-all"><Target size={18} className="text-amber-400" /> Visi & Misi</Link>
                   <Link onClick={() => setIsMenuOpen(false)} href="/profil/sejarah" className="flex items-center gap-3 text-lg font-bold text-emerald-100 no-underline hover:text-white transition-all"><History size={18} className="text-amber-400" /> Sejarah Sekolah</Link>
                </div>

                <Link onClick={() => setIsMenuOpen(false)} href="/ppdb" className="mt-8 block bg-amber-400 text-emerald-950 p-5 rounded-2xl text-center font-black text-lg no-underline shadow-xl hover:bg-white transition-all uppercase italic">Daftar Sekarang</Link>
              </div>
            </motion.div>
          </>
        )}
      ...
      {/* 4. HERO SECTION - Ditambah padding top biar nggak ketutup navbar */}
      <section className="relative min-h-screen flex items-center justify-center pt-32 md:pt-40 overflow-hidden bg-[radial-gradient(circle_at_top_right,_var(--tw-gradient-stops))] from-emerald-50 via-white to-white text-left">
          ...
          <div className="flex flex-col md:flex-row items-center justify-between gap-12">
            <div className="md:w-3/5 text-center md:text-left">
              ...
              <div className="flex flex-wrap gap-4 justify-center md:justify-start">
                ...
                <button onClick={() => setIsMenuOpen(true)} className="bg-white text-slate-800 border-2 border-slate-100 px-10 py-5 rounded-2xl font-black text-sm no-underline uppercase tracking-widest hover:bg-slate-50 transition-all cursor-pointer">Jelajahi Menu</button>
              </div>
            </div>
            <div className="md:w-2/5 relative">
              ...
              <motion.div animate={{ y: [0, -15, 0] }} transition={{ duration: 4, repeat: Infinity }} className="absolute -left-5 bottom-20 bg-white/95 backdrop-blur-xl p-6 rounded-[24px] shadow-2xl border border-white/50 z-30 hidden md:block">
      ...
      <section className="py-24 px-6 bg-white overflow-hidden">
        <div className="max-w-6xl mx-auto flex flex-col md:flex-row items-center gap-16 text-left">
          <div className="w-full md:w-1/2 relative">
            ...
            <div className="relative h-[400px] md:h-[500px] w-full rounded-[40px] overflow-hidden shadow-2xl border-4 border-white">
